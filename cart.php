...
<html lang="en">
  ...
  <body>
    ...
    <div class="carttable">
      <?php 
        // check if cart is empty, if empty, show empty page, if not, show cart
        include 'config.php';
        $query = "SELECT * FROM cart";
        $result = mysqli_query($connection, $query);
        if(mysqli_num_rows($result) == 0) {
            echo '      <h2 class="mb-5 ms-2">My Cart</h2>
            <h5 class="mb-4 pt-5 mt-5" style="text-align: center">YOUR CART IS EMPTY</h5>
            <p style="text-align: center">
              <a type="button" href="index.php#product" class="btn btn1 btn-warning">Buy our product</a>
            </p>
            <!-- akhir cart kosong -->';} else {
      ...
      <div class="container">
        ...
        <div class="row carttotal">
          ...
          <div class="col-3" style="position: relative">
            <div class="plusmin" style="position: relative">
              <button type="submit" class="btn btn-default btn-number remove2 mx-4" name="update[<?= $row['product_id'] ?>]"><i class="bi bi-arrow-repeat"></i></button>
              <div class="center mt-1">
                <div class="input-group input2">
                  <span class="input-group-btn">
                    <button type="button" class="btn btn-default btn-number" id="minus">-</button>
                  </span>
                  <input type="text" id="quantity" class="form-control input-number inputt" value="<?= $row['quantity'] ?>" min="1" name="quantity[<?= $row['product_id'] ?>]" />
                  <span class="input-group-btn">
                    <button type="button" class="btn btn-default btn-number" id="add">+</button>
                  </span>
                </div>
                <input type="text" name="id[<?= $row['product_id'] ?>]" id="id" value="<?= $row['product_id'] ?>" hidden />
              </div>
              <button id="removebutton" type="submit" class="remove2" name="remove[<?= $row['product_id'] ?>]"><i class="bi bi-trash3-fill buang" style="font-size: 24px"></i></button>
            </div>
        ...
        <?php
              ...
              if(isset($_POST['remove'][$row['product_id']])) {
                $id = $row['product_id'];
              ...
              // if(isset($_POST['update'][$row['product_id']]) && isset($_POST['id'][$row['product_id']])) {
              if(isset($_POST['update'][$row['product_id']])) {
                $id = $row['product_id'];
        ...
        <div class="row border-top border-dark-subtle mt-4 pt-2 justify-content-between">
          ...
          <div class="col-3">
            ...
            <p style="color: #6c7c8c">Shipping calculated at checkout</p>
        ...
        <?php 
          ...
          if(isset($_POST['submit'])) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
              $id = $row['product_id'];
              echo $id;
              $query2 = "UPDATE cart SET status = '1' WHERE product_id = $id";
